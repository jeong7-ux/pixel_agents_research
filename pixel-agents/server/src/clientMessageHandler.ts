...
function handleWebviewReady(send: WsSend, ctx: ClientMessageContext): void {
  ...
  // 3. Restore persisted external agents (standalone only; VS Code handles its own restore)
  runtime?.restoreExternalAgents();

  // 4. Existing agents (either just restored, or from VS Code adapter if present)
  ...
  // 5. Layout (saved file, or bundled default)
  const savedLayout = readLayoutFromFile();
  send({ type: 'layoutLoaded', layout: savedLayout ?? cache?.defaultLayout ?? null });

  // 6. Settings (from adapter, with sensible defaults when adapter is absent)
  const cfg = readConfig();
