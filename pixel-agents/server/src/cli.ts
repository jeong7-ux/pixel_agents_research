...
import { copyHookScript, customProvider, openrouterProvider } from './providers/index.js';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const onSetHooksEnabled = async (enabled: boolean): Promise<void> => {
      ...
      if (enabled) {
        await customProvider.installHooks(
        ...
        await customProvider.uninstallHooks();
    ...
    if (runtime.hooksEnabled.current) {
      try {
        await customProvider.installHooks(`http://127.0.0.1:${config.port}`, config.token);
        copyHookScript(distRoot);
